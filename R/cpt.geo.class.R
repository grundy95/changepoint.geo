setClass('cpt.geo',representation(data.set='matrix',distance='numeric',angle='numeric',penalty='character',pen.value='numeric',test.stat='character',msl='numeric',nquantiles='numeric',dist.cpts='numeric',ang.cpts='numeric', date='character',version='character'),prototype=prototype(date=date()))
...
setMethod('plot','cpt.geo',function(x,cpt.col='blue',cpt.width=1,cpt.style=1,...){
		  par(mfrow=c(2,1))
		  plot(distance(x),ylab='Distance',...)
		  abline(v=dist.cpts(x),col=cpt.col,lwd=cpt.width,lty=cpt.style)
		  plot(angle(x),ylab='Angle',...)
		  abline(v=ang.cpts(x),col=cpt.col,lwd=cpt.width,lty=cpt.style)
})
