setClass('cpt.geo',representation(data.set='matrix',mapped.data='matrix',dist.measure='character',penalty='character',pen.value='numeric',test.stat='character',msl='numeric',nquantiles='numeric',dist.cpts='numeric',ang.cpts='numeric', date='character',version='character'),prototype=prototype(date=date()))
...
if(!isGeneric('mapped.data')){
	if(is.function('mapped.data')){
	fun <- mapped.data
	}
	else{
		fun <- function(object){
			standardGeneric('mapped.data')
		}
	}
	setGeneric('mapped.data',fun)
}
setMethod('mapped.data','cpt.geo',function(object) object@mapped.data)

if(!isGeneric('dist.measure')){
	if(is.function('dist.measure')){
		fun <- dist.measure
	}
	else{
		fun <- function(object){
			standardGeneric('dist.measure')
		}
	}
	setGeneric('dist.measure',fun)
}
setMethod('dist.measure','cpt.geo',function(object) object@dist.measure)
...
if(!isGeneric('dist.ts')){
	if(is.function('dist.ts')){
		fun <- dist.ts
	}
	else{
		fun <- function(object){
			standardGeneric('dist.ts')
		}
	}
	setGeneric('dist.ts',fun)
}
setMethod('dist.ts','cpt.geo',function(object) as.ts(object@mapped.data[,1]))

if(!isGeneric('ang.ts')){
	if(is.function('ang.ts')){
		fun <- ang.ts
	}
	else{
		fun <- function(object){
			standardGeneric('ang.ts')
		}
	}
	setGeneric('ang.ts',fun)
}
setMethod('ang.ts','cpt.geo',function(object) as.ts(object@mapped.data[,2]))
...
setGeneric('mapped.data<-', function(object,value) standardGeneric('mapped.data<-'))
setReplaceMethod('mapped.data','cpt.geo',function(object,value){
			 object@mapped.data <- value
			 return(object)
})
setGeneric('dist.measure<-', function(object,value) standardGeneric('dist.measure<-'))
setReplaceMethod('dist.measure','cpt.geo',function(object,value){
			 object@dist.measure <- value
			 return(object)
})
...
setGeneric('dist.ts<-', function(object,value) standardGeneric('dist.ts<-'))
setReplaceMethod('dist.ts','cpt.geo',function(object,value){
			 object@dist.ts <- value
			 return(object)
})
setGeneric('ang.ts<-', function(object,value) standardGeneric('ang.ts<-'))
setReplaceMethod('ang.ts','cpt.geo',function(object,value){
			 object@ang.ts <- value
			 return(object)
})
...
#		   cat('Created using changepoint.geo version',object@version,'\n')
		   cat('Distance Measure	: ',dist.measure(object),'\n')
...
#Plot function
setMethod('plot','cpt.geo',function(x,cpt.col='blue',cpt.width=1,cpt.style=1,...){
		  par(mfrow=c(2,1))
		  plot(dist.ts(x),ylab='Distance',...)
		  abline(v=c(dist.cpts(x)),col=cpt.col,lwd=cpt.width,lty=cpt.style)
		  plot(ang.ts(x),ylab='Angle',...)
		  abline(v=c(ang.cpts(x)),col=cpt.col,lwd=cpt.width,lty=cpt.style)
})
